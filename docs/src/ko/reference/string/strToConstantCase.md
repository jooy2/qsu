...
::: warning 대문자로 바꿀 때 길어지는 글자 JavaScript와 Python은 유니코드 전체 케이스 매핑(full case mapping)을, Dart는 단순 케이스 매핑(simple case mapping)을 사용합니다. 그래서 대문자로 바뀔 때 두 글자로 늘어나는 일부 문자에서 결과가 달라집니다. `straße`는 JavaScript와 Python에서 `STRASSE`가 되지만 Dart에서는 `STRAßE`가 됩니다. `ﬁ` 같은 합자도 마찬가지입니다. Dart에서 동일하게 맞추려면 유니코드 특수 케이스 매핑 표를 그대로 들고 있어야 하므로, 억지로 통일하지 않고 차이를 그대로 두었습니다. :::
