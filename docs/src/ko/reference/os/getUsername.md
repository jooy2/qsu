...
<PlatformSupport :rows="[
	...
	{ os: 'android', note: '앱 프로세스를 실행하는 사용자이며, 안드로이드는 사람이 아니라 앱 이름을 씁니다.' },
	{ os: 'ios', support: { js: 'no', dart: 'yes', python: 'no' }, note: { js: 'JavaScript와 Python 패키지는 iOS에서 동작하지 않습니다.', dart: '`mobile`이며, 기기의 모든 앱이 이 사용자로 실행됩니다.', python: 'JavaScript와 Python 패키지는 iOS에서 동작하지 않습니다.' } }
]" />
