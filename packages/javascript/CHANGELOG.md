...
- `parseAddress`: `path`, `query`, `params` and `hash` were added. Everything from the first `/`, `?` or `#` used to be dropped, so `file:///etc/hosts` reported nothing but its scheme. `query` is the raw string and `params` is it read into an object, where a repeated key keeps its last value and a pair with no key is left out
...
- `getLocalIp`: Added. The IPv4 address this machine is reachable at on its own network. It is the address behind the default route, so a machine with several interfaces answers with the one that would carry traffic out rather than the first one configured. Nothing is sent to find it: a UDP socket is connected, which only asks the kernel which route it would take, and closed again
- `getMacAddress`: Added. The hardware address of the first network interface that is not loopback. macOS gives Wi-Fi a randomised address by default, so the value there changes between networks and is not the address printed on the machine. There is no Python port: its standard library reaches for a different interface than this runtime does, and two packages disagreeing about the same machine is worse than one of them not answering
- `getProcessMemoryUsage`: Added. The physical memory this process occupies right now, as readable text with its unit. It is the resident set, which is the figure a task manager shows against the process name
- `getTimezone`: Added. The IANA name of the system time zone, such as `Asia/Seoul`, which is what a date library takes. An offset cannot say when the clocks change, so the name is what is reported. There is no Python port: its standard library has no way to resolve the name, and Windows would need a mapping table of its own on top
...
- `getDiskSize`, `getFreeDiskSize`, `getDiskUsage`: The block size is read from `bsize` rather than `frsize`. Node only began reporting `frsize` in v26, so on every earlier runtime the multiplication produced `NaN` and the two size functions returned `NaN undefined`. libuv fills `frsize` from `bsize` on every platform but Linux, where the two are the same number on any filesystem in ordinary use, so the value is unchanged where it worked
...
- The package now requires Node.js 20 or newer, where it declared 18. Node 18 reached end of life in April 2025, and the os functions added in this version call `os.availableParallelism()` and `fs.statfs()`, neither of which existed in 18.0. The test matrix now runs the lowest supported version as well as the current ones, which it did not before
- `getPlatform`: Android is reported as `android` rather than as `linux`. The Dart package runs there and answers that way, and what a program is allowed to do differs enough between the two that folding one into the other hid something the caller needs to know. `ios` joins the list for the same reason
...
- `objMerge`: Added. Merges any number of objects into one new object, going down through nested objects, with the later source winning. Two plain objects under the same key are merged into a new object, so neither source is shared with the result or modified. Arrays are replaced whole rather than merged index by index as Lodash does, and `null` is returned when an argument is not an object
