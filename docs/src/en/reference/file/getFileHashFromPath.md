...
::: warning `md5` is the default because this function is most often used to tell two files apart. It is broken as a cryptographic hash — a collision can be constructed on purpose — so use `sha256` when the answer has to be trusted against a deliberate attempt to forge it. :::
