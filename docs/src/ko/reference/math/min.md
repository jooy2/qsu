...
숫자가 아닌 값은 `sum`과 동일하게 건너뜁니다. `NaN`도 건너뛰는데, 비교에서 항상 지기 때문에 그대로 두면 처음 등장한 뒤 교체되지 않아 결과가 되어버리기 때문입니다.

::: lang js dart

비교할 값이 하나도 없으면(빈 배열이거나 인수가 없는 경우) `null`을 반환합니다.

:::

::: lang python

비교할 값이 하나도 없으면(빈 리스트이거나 인수가 없는 경우) `None`을 반환합니다.
