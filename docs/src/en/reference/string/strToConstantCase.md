...
::: warning A letter that grows when uppercased JavaScript and Python apply the full Unicode case mapping, where Dart applies the simple one. A handful of letters that expand into two characters therefore differ: `straße` becomes `STRASSE` in JavaScript and Python, but `STRAßE` in Dart. The same holds for ligatures such as `ﬁ`. This is not papered over, because matching it would need a copy of the Unicode special-casing table in Dart. :::
